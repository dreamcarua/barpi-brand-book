...
## Лендінг Kormotech — будова, якої більше не видно з коду

`kormotech/index.html` (99 621 б, 25 слайдів) + `kormotech/img/*.webp`. Публікується як частина Pages; закритий лише рядком `Disallow: /kormotech/` у `robots.txt` — це «неіндексований», а не «захищений».

Як воно збиралось до 04.09.2026 (генератори втрачено разом з контейнером, див. `docs/traps.md`):

- `build_deck.py` був єдиним джерелом правди для **обох** артефактів. `build_landing.py` робив `exec()` його вмісту до рядка `def render_slide(s):` і перепаковував ті самі `SLIDES` у скрол-секції. Правити треба було тільки `build_deck.py`; `build_landing.py` — лише лендінг-специфічне: `NAV`, `IDMAP`, hero-обгортка, адаптив.
- **Три білди на кожну зміну:** дек 1280×720; лендінг локальний (`DEPLOY=` → base64, self-contained, 160 281 б); лендінг деплойний (`DEPLOY=1` → відносні шляхи `img/…`, 99 621 б). Забути другий = на прод поїде 160 КБ base64 або зламані картинки.
- **CI, який збирав би HTML, у репо немає** — `index.html` комітиться готовим. Це і є причина втрати генератора.
- Хук `_PROD_FILES = []` для фото з виробництва був у `build_deck.py`. У поточному HTML hero текстовий; CSS-правила `.title-photo` і `.hero-prod` в стилях лишились, розмітки немає.
- Перевірка прода: `curl -s -o /tmp/k.html -w "%{http_code} %{size_download}"` + grep по маркерах. Розмір відповіді має байт-у-байт збігатися з локальним деплойним білдом — найшвидший доказ, що задеплоїлось саме те.
- Playwright: Python-версія в `/usr/local/lib/python3.11/dist-packages/playwright`, Chromium через `glob.glob('/opt/pw-browsers/chromium-*/chrome-linux/chrome')[0]`. Node-версія дає `ERR_MODULE_NOT_FOUND` — не використовувати.
