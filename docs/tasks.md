...
- **Kormotech: генератор `build_deck.py` / `build_landing.py` втрачено** — рішення власника 04.09.2026: **не відбудовувати, поки лендінг працює**. Джерело правди — `kormotech/index.html` у git; правки вручну у двомовних `data-lang` спанах (структура `L()` збереглася в HTML). Відбудовувати тільки якщо знадобиться серія змін. [свідомо відкладено]
