...
- **Kormotech: вирішити долю генератора** — `build_deck.py` і `build_landing.py` втрачено разом з контейнером (04.09.2026). Лендінг живий і актуальний. Наступний крок: або відбудувати генератор з `kormotech/index.html`, або перейти на пряме редагування HTML і закомітити це рішення в README. Поки не вирішено, кожна правка робиться вручну у двомовних `data-lang` спанах.
- **Kormotech: фото з виробництва на головний екран** — Олександр, ~25.07.2026: «вместо этого слайда контент с производства (Работа с мясом)», «можно выбрать 1-2 фото». Наступний крок: отримати файли, покласти в `kormotech/img/` через `git push` з машини власника (не через MCP — бінар псується), додати `<div class="hero-photo hero-prod">` у hero-секцію.
- **Kormotech: сторінка з комерційною пропозицією захищена лише `robots.txt`** — репо публічне, сторінка доступна за прямим посиланням. Наступний крок: рішення власника, чи переносити її за CF Access, як `/dashboard/*`.
