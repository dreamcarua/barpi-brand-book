...
## v7.4 (06.09.2026) — 👁 Видимість збереження у Базі знань

Причина: звіт «вчора додавали дані, а зараз їх не бачимо». Перевірка D1 показала 0 зниклих відповідей і +8 доданих — втрати не було. Проблема була в інтерфейсі: набраний текст автозберігався лише в `localStorage` браузера і сторінка про це мовчала.

- **Панель «🕘 Останні зміни в базі»** над списком питань: 10 останніх подій (нова відповідь · правка · файл) з автором, часом і переходом на картку питання. Оновлюється після кожного збереження, знімає фільтр і підсвічує картку при кліку.
- **Бейдж «⚠️ не збережено в базі»** біля кнопки, поки текст лежить у полі і не відправлений; плюс попередження браузера при спробі закрити вкладку з незбереженим текстом.
- **Помилка збереження більше не зникає через 3 с** — червона плашка з поясненням («текст залишився у полі і в чернетці браузера») і кнопкою «Спробувати ще раз» тримається, поки збереження не пройде.

Дані і API не змінювались — тільки поведінка інтерфейсу.

## v7.3 (21.08.2026) — 📎 Файли у Базі знань + чек-лист поля + пріоритетні виконавці

### 📎 Вкладення (нове)
- **Файли до кожного питання** — R2 bucket `barpi-kb-files`, до 20 МБ, allowlist розширень (док/таблиці/презентації/зображення/відео/архіви). Завантаження кнопкою або **drag & drop на картку**, прогрес-бар, скачування, видалення (автор або vg).
- Безпека: ім'я файлу санітизується (path traversal → базове ім'я, контрольні символи і лапки вирізано), R2-ключ будується з UUID, віддача завжди `Content-Disposition: attachment` + `nosniff`, guard 60 завантажень/год, allowlist авторів. Soft-delete у D1, об'єкт у R2 лишається (відновлюваність).

### ✅ Поля чек-листа SC (нове)
- `kb_question_state`: **Стан** (Є / В роботі / Не стосується), **Де зберігається / ресурс**, **Відповідальний** — тепер сторінка збирає рівно ті колонки, які просить SC Consulting. Автозбереження, видно хто оновив.
- Прогрес рахується коректно: питання зі статусом «не стосується» виключені зі знаменника.

### 👤 Пріоритетні виконавці (нове)
- Колонка «Відповідальний» із заповненого чек-листа (В/А/Ф) імпортована у `kb_questions.resp_code` для всіх 48 питань. Бейдж біля кожного питання + зведення у заголовку блоку; свої питання підсвічені навігаційно, фільтр **«Мій блок»**.
- Пункти 22, 40, 48 («Не применимо» у чек-листі) автоматично отримали статус «не стосується».

### 🔀 Інше
- **Перемикач порядку**: «№ 1→48» (як у чек-листі) ↔ «За темами» (6 блоків), вибір запам'ятовується.
- **Експорт .csv** у колонках оригінального чек-листа (№ · Задача · Є/немає · Де зберігаються · Відповідальний · Примітка), BOM для Excel. Експорт .md доповнено станом, файлами і пріоритетним виконавцем.
- Пошук тепер шукає і по іменах файлів, і по полю «Де зберігається».
- Worker `barpi-api`: `POST /kb_files`, `GET /kb_files/:id/download`, `DELETE /kb_files/:id`, `PUT /kb_state/:qid`; нові таблиці додано у weekly backup і `/export`. Прив'язано R2 `KBFILES`.
